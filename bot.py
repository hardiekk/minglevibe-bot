...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("buy_"):
        ...
        pending_payments[order_id] = {"user_id": user_id, "plan_key": plan_key, "plan_days": plan["days"], "plan_label": plan["label"], "amount": plan["amount"]}
        upi_link = f"upi://pay?pa=kunalminglevibe@fam&pn=MingleVibe&am={plan['amount']}&cu=INR&tn={order_id}"
        await query.edit_message_text(
            f"💳 Payment Details\n\nPlan: {plan['label']} - {plan['display']}\nOrder ID: {order_id}\n\nHow to Pay:\n1. GPay / PhonePe / Paytm kholo\n2. Pay {plan['display']} to: kunalminglevibe@fam\n3. Remark mein likho: {order_id}\n4. Neeche I've Paid tap karo",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("📲 Open GPay", url=f"https://gpay.app.goo.gl/pay")],
    ...
    if data.startswith("paid_"):
        ...
        if not order:
            await query.edit_message_text("Order not found. Contact: @hardieezson")
            return
        await query.edit_message_text(
            f"Payment submitted!\nOrder: {order_id}\nPlan: {order['plan_label']}\n\nAdmin 30 min mein verify karega.\nContact: @hardieezson"
        )
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(admin_id, f"New Payment!\nUser: {user_id}\nPlan: {order['plan_label']}\nAmount: Rs.{order['amount']}\nOrder: {order_id}\nRun: /givepremium {user_id} {order['plan_days']}")
            except Exception:
                pass
        return
        return
